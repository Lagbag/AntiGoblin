...
0. Keenetic → «Приоритеты подключений» → «Политики доступа в интернет»: у `xkeen` в колонке **«Подключение» должен быть WAN**, а не пустое место. Пустая колонка означает, что policy существует и клиенты могут быть к ней привязаны, но в самой policy отсутствует `permit global <WAN>`. Начиная с runtime2 AntiGoblin чинит это автоматически при update/apply/self-heal, не пересоздавая policy и не снимая клиентов.
...
### Быстрая проверка: VPN жив или проблема только в policy

```sh
# Через сам активный VPN, обходя назначение клиентов в Keenetic
/opt/bin/curl -4 -fsS --socks5-hostname 127.0.0.1:61080 https://api.ipify.org

# Напрямую через WAN роутера
/opt/bin/curl -4 -fsS https://api.ipify.org

# Policy-блоки; у xkeen ожидается permit global <интерфейс>
ndmc -c 'show running-config' | sed -n '/^ip policy /,/^!/p'
```

- Первый IP есть и отличается от второго → VPN-апстрим рабочий; ищем проблему в policy/перехвате.
- Первый запрос не проходит → routing.json тут ни при чём; проверять активный ключ и `xray-manual.log` / `sing-box-xkeen.log`.
- IP одинаковые → SOCKS-путь не подтверждает смену выхода; UI runtime2 показывает это как предупреждение после Apply.
