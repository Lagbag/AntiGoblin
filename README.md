...
```sh
git add -A
git commit -m "feat: runtime5 dashboard and verified VPN server selection"
git push origin main
```


### Что изменилось в runtime5

- Интерфейс полностью разбит на пять нормальных разделов: **Обзор / Серверы / Маршруты / Диагностика / Система**. Больше нет одной длинной страницы, где подписки, routing.json и логи лежат вперемешку.
- На «Обзоре» крупно показывается **сервер, который реально применён в runtime**, его protocol/engine/endpoint, состояние SOCKS `:61080`, счётчик `xkeen`, WAN policy и авто-выбор. Выбранный в UI и реально применённый сервер теперь различаются.
- В списке серверов видны протокол, движок (`xray` / `sing-box`), endpoint, RTT, метка **«применён»** и кнопка **«Применить»** для выбранного, но ещё не применённого узла.
- Кнопка «Проверить активный» теперь делает **реальный HTTPS egress-check через `127.0.0.1:61080`**, а не только TCP/ping до адреса сервера.
- Диагностика показывает, где ломается цепочка `xkeen → Xray → sing-box → сервер`; HY2 `AUTH 404`, QUIC timeout и TLS error выводятся как разные состояния.
- Нижняя плашка больше не висит постоянно: она появляется только во время Apply или для результата/ошибки и затем скрывается. Основная кнопка Apply находится в шапке.
- Подписки запрашиваются с `User-Agent: sing-box`, чтобы Hiddify Manager чаще отдавал нативный sing-box/Hiddify вариант, который AntiGoblin умеет импортировать без потери provider-specific полей.
- Автовыбор по-прежнему ранжирует все узлы по RTT, но переключение считается успешным только после настоящего HTTPS через туннель; мёртвый HY2/TUIC с хорошим ping не должен становиться рабочим сервером.
- Наследуются исправления runtime3/runtime4: HY2 port-hopping/ECH, корректная обработка `pinSHA256`, RAM runtime-log sing-box и диагностические `AUTH 404`.

> Если старый HY2 был импортирован до исправлений парсера, после обновления нажми ↻ у подписки (или заново вставь исходный URI), затем `Сохранить и применить`: потерянные при старом импорте параметры нельзя восстановить из уже урезанного state.
