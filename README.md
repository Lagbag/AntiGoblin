...
Если ты заменил содержимое своего fork файлами из релизного ZIP, опубликовать обновление можно одним блоком:

```sh
git add -A
git commit -m "feat: runtime6 resilient failover and diagnostics UX"
git push origin main
```


### Что изменилось в runtime6

`runtime6` — это предрелизная доводка перед реальным тестом на роутере: меньше скрытых состояний, меньше ложных диагнозов и меньше ручного SSH.

- UI по-прежнему разделён на **Обзор / Серверы / Маршруты / Диагностика / Система**, но теперь явно показывает **неприменённые изменения**. Если выбрал другой сервер или поправил правила, в шапке появляется предупреждение и кнопка меняется на `Применить изменения`; при попытке закрыть вкладку браузер предупреждает о несохранённом runtime.
- Dashboard хранит отдельную метку **какая версия AntiGoblin и какой `activeProxyId` реально создали текущий runtime**. После in-place update старая конфигурация больше не выглядит как полностью синхронизированная: UI попросит один раз нажать Apply.
- Совпадение `выбран` / `применён` теперь определяется прежде всего по стабильному ID ключа, а не только по `protocol + host + port`; два ключа на одном endpoint с разными credentials больше не должны визуально путаться.
- Автовыбор получил **quarantine/cooldown** для узлов, которые пингуются, но не проходят настоящий HTTPS через туннель. HY2 `AUTH 404`/битый config исключается на час, TLS — на 30 минут, QUIC timeout и обычный egress-fail получают короткий backoff. Причина и оставшееся время видны прямо рядом с сервером.
- После успешного ручного Apply quarantine текущего сервера очищается. Просроченный backoff начинает новый failure-streak с нуля, а не наращивается бесконечно.
- Исправлена важная диагностика: `/tmp/antigoblin-singbox-runtime.log` очищается перед каждым новым sing-box runtime/candidate, поэтому старый HY2 `AUTH 404` не может ошибочно приклеиться к следующему VLESS/TUIC/SS серверу.
- При старте после reboot авто-селектор делает до трёх быстрых повторов через 20 секунд, если WAN/DNS/Entware ещё не успели полностью подняться; после этого возвращается к обычному интервалу, чтобы не устраивать hot-loop.
- Если авто-селектор успел поднять новый сервер, но не смог атомарно записать новый `activeProxyId` в state, runtime откатывается назад. UI и реально работающий сервер больше не должны расходиться из-за частичной записи.
- Apply сначала проверяет нейтральный `https://example.com` через SOCKS. `api.ipify.org`/`icanhazip.com` используются только как дополнительная информация; блокировка одного public-IP сервиса больше не превращается в ложное «VPN не работает».
- В **Диагностике** появилась кнопка **«Скопировать отчёт»**. Она сама собирает health, применённый/выбранный сервер, egress-check, auto-select status и последние логи Xray/sing-box/auto-select, при этом вырезает URI/UUID и не копирует public IP. Это специально сделано, чтобы не вставлять огромные многострочные команды в BusyBox.
- Наследуются multiprotocol/Hiddify исправления: VLESS/VMess через Xray, HY2/Hysteria/TUIC/Trojan/SS/AnyTLS и др. через sing-box bridge, raw sing-box/Hiddify JSON, HY2 port-hopping/ECH, современный WireGuard endpoint и transactional Apply.

> Если старый HY2 был импортирован до исправлений парсера, после обновления нажми ↻ у подписки (или заново вставь исходный URI), затем `Применить изменения`: потерянные при старом импорте параметры нельзя восстановить из уже урезанного state.

#### Быстрый утренний smoke-test

1. Обновить AntiGoblin и сделать `Ctrl+F5`.
2. На **Обзоре** проверить версию и нажать `Применить изменения` один раз, если UI пишет, что runtime создан старой версией.
3. На **Серверах** выбрать заведомо рабочий VLESS и нажать `Применить`; на Обзоре строка `Применено сейчас` должна поменяться именно на этот ID/endpoint.
4. Нажать `Проверить активный`: ожидается `VPN OK` / HTTPS 2xx.
5. Включить авто-выбор и нажать `Проверить все`. Узел с `AUTH 404` должен получить красную причину + cooldown, а selector — попробовать следующий проверенный узел.
6. Открыть **Диагностика → Скопировать отчёт**. Если что-то не работает, этого отчёта обычно достаточно для разбора без SSH-простыни.
...
6. В Keenetic web UI назначить нужные устройства в политику `xkeen` в разделе «Приоритеты подключений». В таблице **«Политики доступа в интернет»** у строки `xkeen` колонка **«Подключение» не должна быть пустой**: AntiGoblin автоматически разрешает в этой политике текущий WAN (`permit global <WAN>`) и при обновлении чинит старую политику на месте, не сбрасывая назначенных клиентов.
...
В этой версии `Сохранить и применить` больше не считает работу успешной только потому, что JSON записался. Backend одной транзакцией валидирует будущие Xray/sing-box конфиги, перезапускает ядра, проверяет/восстанавливает WAN внутри политики `xkeen`, устанавливает `iptables nat PREROUTING → xkeen → REDIRECT :61219`, а затем делает **реальный HTTPS-запрос через `127.0.0.1:61080`**. Если локальный SOCKS не может выйти наружу или его публичный IP совпадает с прямым WAN-IP, UI показывает предупреждение вместо безусловного зелёного «Применено». Тяжёлый DNS/ipset refresh затем запускается self-heal отдельно.

Если в Keenetic → **«Приоритеты подключений» → «Политики доступа в интернет»** у `xkeen` пустая колонка «Подключение», это отдельная проблема от `routing.json`: у policy нет разрешённого глобального WAN. Текущий installer/update и self-heal ремонтируют её автоматически, сохраняя саму политику и привязанных к ней клиентов.
...
# Для сравнения — прямой публичный IP самого роутера
/opt/bin/curl -4 -fsS https://api.ipify.org

# Проверить policy-блоки Keenetic: внутри xkeen должен быть `permit global <WAN>`
ndmc -c 'show running-config' | sed -n '/^ip policy /,/^!/p'
...
Если SOCKS-команда показывает VPN-IP, но счётчик `xkeen` остаётся `0`, назначь тестовое устройство политике Keenetic **`xkeen`** в `Приоритеты подключений`. Если счётчик растёт, но SOCKS-команда не возвращает IP — проблема уже в активном VPN-ключе/сервере, а не в правилах Xray. Если у `xkeen` в Keenetic пустая колонка «Подключение», обновись и нажми `Сохранить и применить`: runtime восстановит WAN в policy. UI различает эти состояния отдельными проверками.
